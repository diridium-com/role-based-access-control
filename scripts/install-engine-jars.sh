...
#
# NOTE: the engine build has moved from Ant to Gradle, but the Gradle build was
# set up to emit to the same "setup" output dirs the Ant build used (donkey/setup
# and server/setup/{server-lib,client-lib}), so the JAR paths below are current
# for a Gradle checkout. Build the engine (its Gradle build) first so these jars
# exist, then run this script.
...
for entry in "${JARS[@]}"; do
    ...
    if [[ ! -f "${jar_path}" ]]; then
        ...
        echo "  build the engine (its Gradle build) first so its jars exist under the" >&2
        echo "  'setup' output dirs, then re-run this script." >&2
        exit 1
    fi
done
